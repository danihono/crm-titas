...
import { saveContact, type NewContactForm } from '../../hooks/useContacts'

const EMPTY: NewContactForm = { name: '', role: '', company: '', email: '', phone: '', whats: '' }

export default function ContactModal({ onClose, onSaved }: { onClose: () => void; onSaved: (id: string) => void }) {
  ...
  return (
    <Modal width={500} onClose={onClose}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <div style={{ ...sx.serif, fontSize: 23, fontWeight: 600, color: '#1d1726' }}>Novo contato</div>
        <MaterialIcon name="close" size={23} color="#9c95a8" style={{ cursor: 'pointer' }} onClick={onClose} />
      </div>
      <div style={{ fontSize: 12.5, color: '#6e6780', marginBottom: 18 }}>Cadastre todas as informações do cliente.</div>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
        ...
        <button onClick={handleSave} style={{ background: 'linear-gradient(140deg,#7a52a0,#553578)', border: '1px solid rgba(200,160,230,0.3)', borderRadius: 11, padding: '10px 20px', color: '#f4eefa', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: busy ? 0.7 : 1 }}>Salvar contato</button>
      </div>
    </Modal>
  )
}
