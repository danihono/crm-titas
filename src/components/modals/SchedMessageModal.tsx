...
export default function SchedMessageModal({ contactId, contactName, schedule, onClose, onSaved }: { contactId: string; contactName: string; schedule?: ScheduledMessage | null; onClose: () => void; onSaved: (dayKey: string) => void }) {
  const isEdit = !!schedule
  const [text, setText] = useState(schedule?.text ?? '')
  const tomorrow = new Date()
  tomorrow.setDate(tomorrow.getDate() + 1)
  const [date, setDate] = useState(schedule ? dateKeyOf(schedule.dueAt) : dateKeyOf(tomorrow))
  ...
  async function handleSave() {
    ...
    try {
      if (schedule) {
        await updateScheduledMessage(schedule.id, contactName, text, date, time, schedule.eventId)
      } else {
        await saveScheduledMessage(contactId, contactName, text, date, time)
      }
      onSaved(date)
    } finally {
      setBusy(false)
