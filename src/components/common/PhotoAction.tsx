...
export default function PhotoAction({ icon, title, onClick, disabled, busy, rose, green }: {
  ...
  return (
    <button type="button" title={busy ? t('foto.trabalhando') : title} onClick={onClick} disabled={disabled} style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', borderRadius: 8, background: bg, cursor: disabled ? 'wait' : 'pointer', opacity: disabled && !busy ? 0.5 : 1 }}>
